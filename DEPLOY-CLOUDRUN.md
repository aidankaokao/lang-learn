...
YouTube 封鎖所有雲端供應商（GCP／AWS／Azure）的 IP，**自動擷取字幕在 Cloud Run 上不可能成功**，
所以這個功能已經移除了。匯入影片後請自己貼字幕：

1. 影片庫貼上 YouTube 網址 → 加入
2. 卡片上按 **「貼上字幕」**
3. 在 YouTube 影片頁點「⋯更多」→「顯示轉錄稿」，複製整份內容貼進來
4. 進學習頁後可按 **「重新斷句」** 讓 AI 補標點、切成一句一段

SRT / VTT 檔的內容也吃。**內容必須含時間**，否則無法做 AB 擷取。
