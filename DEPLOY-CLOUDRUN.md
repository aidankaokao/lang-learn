...
### ⚠️ YouTube 會封鎖雲端主機的 IP

**這是必然會遇到的**，不是設定錯誤。YouTube 幾乎封鎖所有雲端供應商（GCP／AWS／Azure）的 IP，
所以在 Cloud Run 上匯入影片時，字幕擷取會失敗並顯示 `RequestBlocked`。本機開發沒事，
是因為你家是住宅 IP。
...
匯入失敗的影片卡片上會出現 **「貼上字幕」** 按鈕：

1. 在瀏覽器打開那支 YouTube 影片
2. 影片說明欄下方 **「...更多」→「顯示轉錄稿」**
3. 全選轉錄稿面板的內容複製（會像 `0:05` + 文字這種格式）
4. 貼進「貼上字幕」的輸入框 → **匯入字幕**

SRT / VTT 檔的內容也吃。**內容必須含時間**，否則沒有時間軸就無法做 AB 擷取。
匯入後標籤會顯示「手動貼上」，後續所有功能（AB 擷取、聽寫、片語萃取）都一樣能用。

#### 解法 2：掛住宅代理（要付費，但全自動）

買一個住宅代理服務（例如 Webshare 的 residential proxy，月費約幾美元），
然後在 Cloud Run 加一個環境變數：

| 環境變數 | 值 |
|---|---|
| `YOUTUBE_PROXY` | `http://使用者:密碼@主機:埠` |

字幕擷取與 yt-dlp 都會自動走這個代理。**注意**：一般的資料中心代理沒有用，
要買**住宅（residential）**代理才繞得過。

> 不建議用 cookies 驗證的方式（`youtube-transcript-api` 的 README 也標為 NOT RECOMMENDED），
> YouTube 最終會永久停用那個帳號。
