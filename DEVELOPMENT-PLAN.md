...
| 項目 | 決定 | 理由 / 代價 |
|---|---|---|
| 文字稿來源 | `youtube-transcript-api` 抓英文字幕（含自動字幕）；失敗才 `yt-dlp` 抽音訊 → OpenAI `whisper-1` | 字幕路徑免費且自帶時間軸；Whisper 是 fallback，需 `ffmpeg` |
...
### 練習模式
1. **循環聆聽**（列表上直接用）— 例句列表每筆都有耳機鈕，按了就在 A–B 之間無限循環，
   不必進練習頁。手機上塞著耳機重複聽就是主要用法，所以迷你播放器**刻意保持可見**
   （手機瀏覽器不讓看不見的播放器自動播）。
...
| `videos` | 匯入的影片與文字稿狀態（`transcript_status` / `transcript_source`） |
...
| GET | `/api/videos` | 影片列表（附句數 / 例句數） |
| POST | `/api/videos` | 貼網址匯入，文字稿丟背景抓取（回 `transcript_status=pending`） |
| GET | `/api/videos/{id}` | 單支影片（前端輪詢用） |
| GET | `/api/videos/{id}/segments` | 逐句文字稿 |
| POST | `/api/videos/{id}/retry` | 重新擷取文字稿 |
...
2. **CPU 配置要選「一律配置」**，否則 `BackgroundTasks` 的文字稿擷取會被凍結、永遠停在 pending。
3. **SQLite 不能用**（檔案系統是暫時性的），要接外部 PostgreSQL。
   預設用 **Neon**（serverless，免費方案閒置不計費、不必設 Cloud SQL 連線）；
   要留在 GCP 內則走 Cloud SQL，見該文件附錄 A。切換只改 `DATABASE_URL`。
...
### 文字稿擷取的已知限制（階段 3）
- **不需要 ffmpeg**：Whisper fallback 直接下載 `bestaudio`（m4a / webm）送 API，不做轉檔。
- **Whisper 單檔上限 25MB**，約 30–40 分鐘的音訊。超過會回明確錯誤，要支援長片得先切段（那才需要 ffmpeg）。
- 字幕碎片會**合併成接近句子的段落**（滿 12 秒或 220 字或遇到句號就斷），讓 AB 擷取與逐句對照好用。
- `youtube-transcript-api` 0.6.x 與 1.x 的 API 不同，`_fetch_captions()` 兩種都相容。
- 文字稿擷取走 FastAPI `BackgroundTasks`（同一個 process）。**Cloud Run 若縮到 0 實例會中斷**，
  階段 5 視情況改成外部工作佇列或改用 Cloud Tasks。
