...
- **導覽**：`lg` 以上是常駐側邊欄（`Sidebar`，可折疊、可拖曳調寬）；
  `lg` 以下**沒有側邊欄**，改成一般手機 App 的**頂部下拉選單**（`MobileMenu`）：
  Header 左邊的漢堡鈕開關（展開時變成 ✕），選單從 Header 正下方滑出、佔滿寬度，
  點遮罩、點任一項目或換頁都會收起。導覽項目只有一份（`Sidebar.tsx` 的 `NAV_SECTIONS`），兩邊共用。
  （原本是左側抽屜，開發者回饋希望改成手機常見的上方下拉。）
- **Header**：手機顯示漢堡鈕、桌機顯示折疊鈕；帳號 chip 在 `sm` 以下隱藏，手機改顯示在下拉選單頂端。
